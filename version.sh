...
if [ -n "${_VERSION_TAG_ON_CURRENT_COMMIT}" ]; then
  ...
  echo -n "${_VERSION_TAG_ON_CURRENT_COMMIT#$_COMPONENT_VERSION_PREFIX}"
  exit 0
fi
...
_CURRENT_VERSION="${_LAST_VERSION#$_COMPONENT_VERSION_PREFIX}" # remove leading v
_CURRENT_VERSION=(${_CURRENT_VERSION//./ })                    # replace points, split into array
...
if [ -n "$(git log "${_LAST_VERSION}..HEAD" --oneline --grep="${MAJOR_BUMP_MESSAGE}") --${PATHS}" ]; then
  echo >&2 "Found ${MAJOR_BUMP_MESSAGE} in commit messages, bumping major version."
  echo >&2 $(git log "${_LAST_VERSION}..HEAD" --oneline --grep="${MAJOR_BUMP_MESSAGE}")
  _CURRENT_VERSION[0]=$((_CURRENT_VERSION[0] + 1))
  _CURRENT_VERSION[1]=0
  _CURRENT_VERSION[2]=0
elif [ -n "$(git log "${_LAST_VERSION}..HEAD" --oneline --grep="${MINOR_BUMP_MESSAGE}") --${PATHS}" ]; then
  echo >&2 "Found ${MINOR_BUMP_MESSAGE} in commit messages, bumping minor version."
  echo >&2 $(git log "${_LAST_VERSION}..HEAD" --oneline --grep="${MINOR_BUMP_MESSAGE}")
...
echo "number_of_changes_since_last_tag=$(git log --oneline ${_LAST_VERSION}..HEAD --${PATHS} | wc -l)"
echo "previous_version=${_CURRENT_VERSION[0]}.${_CURRENT_VERSION[1]}.${_CURRENT_VERSION[2]}"
